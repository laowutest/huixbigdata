package com.huix.huixbigdata.batchgeneratedata.mapReduce.tradestatistics;
...
import com.huix.huixbigdata.batchgeneratedata.util.TimeUtil;
import com.mysql.fabric.xmlrpc.base.Value;


/**
 * StormKafkaProcess用于处理实时交易统计以及排行�?(Top10)
 *
...
public class StormKafkaProcess {
	...
	public static class AccBolt extends BaseRichBolt {
		...
		String markTime = null;// 存入mysql的时�?
		...
		public void execute(Tuple input) {
			...
			String date = TimeUtil.stamp2Date(split[0]);// 将时间戳1481111901765转为时间2016-12-7
														// 19:58:21
			String subdate = date.substring(0, date.length() - 3);// 获取到分�?2016-12-7
																	// 19:58
			...
			balance = balance.add(value);// 60s之间的数值之�?(�?1分钟可能不到60s)
	...
	/**
	 * ToDbBolt用于接收1分钟之内的商品�?�和，插入mysql
	 *
	 * @className ToDbBolt
	 * @author mxlee
	 * @email imxlee@foxmail.com
