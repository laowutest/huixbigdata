...
import com.huix.huixbigdata.batchgeneratedata.redis.RedisMS;
...
/**
 * 模拟抢购任务，每个任务都会连接服务器，修改商品的值，这里使用了CAS协议，保证了多线程下的数据原子�??
 * 
...
public class BuyProductionMem implements Runnable {
	...
	public void run() {
		...
		String productionName = "prod" + nextInt;// 随机得到�?个商品名（key�?

		CASValue<Object> casValue = memcachedClient.gets(productionName);// 得到key对应的CASValue
		long cas = casValue.getCas();// 得到商品对应的数量的版本�?
		Integer value = (Integer) casValue.getValue();// 得到商品对应的数量的�?

		if (value > 0) {
			// 通过cas修改value，如果版本号没变则返回OK修改成功，如果版本号变了则返回其他�??
			CASResponse response = memcachedClient.cas(productionName, cas, value - 1);
			if (response.toString().equals("OK")) {
				System.out.println(
						Thread.currentThread().getName() + "成功抢到�?个商品：" + productionName + "\t剩余�?" + (value - 1));
				count++;
				switch (count) {
				case 1:
				...
				print(Thread.currentThread().getName() + "成功抢到�?个商品：" + productionName + "\t剩余�?" + (value - 1));
			} else {
				System.out.println(Thread.currentThread().getName() + "手�?�慢了，没抢�?");
			}
			System.out.println("时间�?" + System.currentTimeMillis());
		} else {
			System.out.println("商品" + productionName + "，已经被抢光�?");
		}
